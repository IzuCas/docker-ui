...
export default function ChangePasswordPage() {
  const { username: currentUsername, login, token } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await authApi.changePassword(
        currentPassword,
        newPassword,
        usernameChanged ? trimmedUsername : undefined,
      );
      // Update the display name in auth context (token stays valid)
      if (usernameChanged && token) {
    ...
    <div className="min-h-screen bg-bg-primary flex items-center justify-center p-4">
      <div className="w-full max-w-sm">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <h1 className="text-2xl font-bold text-text-primary">Set your credentials</h1>
          <p className="text-text-secondary text-sm mt-1 text-center">
            Choose a username and a secure password before continuing
          </p>
        </div>
        ...
        <div className="bg-bg-secondary border border-border rounded-xl p-6 shadow-xl">
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            {/* New username */}
            <div>
              <label htmlFor="new-username" className="block text-sm font-medium text-text-secondary mb-1.5">
                Username
              </label>
              <input
                id="new-username"
            ...
            <button
              type="submit"
              disabled={loading || !currentPassword || !newPassword || !confirmPassword || !newUsername.trim()}
