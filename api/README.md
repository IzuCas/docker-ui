...
```bash
curl http://localhost:8001/containers?all=true
```
...
# Iniciar
curl -X POST http://localhost:8001/containers/meu-nginx/start
```

### Baixar uma imagem

```bash
curl -X POST http://localhost:8001/images/pull \
  -H "Content-Type: application/json" \
...
```bash
curl -X POST http://localhost:8001/containers/meu-nginx/exec \
  -H "Content-Type: application/json" \
  -d '{"cmd": ["ls", "-la", "/"]}'
```
